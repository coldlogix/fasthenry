...
double **Q2PDiag(ssystem *sys, charge **chgs, int numchgs, int *is_dummy, int calc)
{
  ...
  mat=0;
  mulALLOC(mat, numchgs, double*, ON, AQ2PD, sys,AllocTypePtrArray );

  for(i=0; i < numchgs; i++)
  {
    mulALLOC(mat[i], numchgs, double, ON, AQ2PD, sys, AllocTypeGeneric);
   }
...
double **Q2P(ssystem* sys, charge **qchgs, int numqchgs, int *is_dummy, charge **pchgs,
    int numpchgs, int calc)
{
  ...
  mat=0;
  mulALLOC(mat, numpchgs, double*, ON, AQ2P, sys, AllocTypePtrArray);
  for(i=0; i < numpchgs; i++)
  {
    mulALLOC(mat[i], numqchgs, double, ON, AQ2P, sys, AllocTypeGeneric);
    if(calc)
    {
      /* exclude:
	 ...
	 - dielectric i/f panels in the eval list (if doing 2-point E's)*/
      ...
      {
        if(pchgs[i]->surf->type == DIELEC || pchgs[i]->surf->type == BOTH)
        {
	      continue;
	    }
	  }
      ...
      { /* only dummy panels in the charge list */
	    if(!is_dummy[j])          /* (not the eval list) are excluded */
	    {
	      mat[i][j] = calcp(qchgs[j], pchgs[i], NULL); /* old: pchgs[i],qchgs[j] */
	    }
      }
    }
  }
...
double **Q2Pfull(cube *directlist, int numchgs)
{
  ...
  mat=0;
  CALLOC(mat, numchgs, double*, ON, AQ2P);
  for(i = 0; i < numchgs; i++)
  {
    CALLOC(mat[i], numchgs, double, ON, AQ2P);
  }
...
int compressMat(double **mat, int size, int *is_dummy, int comp_rows)
{
  ...
  if(comp_rows == TRUE || comp_rows == BOTH)
  {
    ...
    for(i = 0; i < cur_order_size; i++)
    {
      ...
      for(j = 0; j < size; j++)
      { /* copy the row to its new location */
	    mat[i][j] = mat[k][j];
      }
    }
  }
  if(comp_rows == FALSE || comp_rows == BOTH)
  {
    ...
    for(j = 0; j < cur_order_size; j++)
    {
      ...
      for(i = 0; i < size; i++)
      { /* copy the col to its new location */
	    mat[i][j] = mat[i][k];
      }
    }
  }
    ...
    int exp_rows)
{
  ...
  if(exp_rows == TRUE || exp_rows == BOTH)
  {
    ...
    for(i = size - 1; i >= 0; i--)
    {
      ...
      {			/* copy the row from its compressed location */
	    for(j = 0; j < size; j++)
  ...
  if(exp_rows == FALSE || exp_rows == BOTH)
  {
    ...
    {
      if(is_dummy[j])
      {		/* zero out dummy column */
	    for(i = 0; i < size; i++)
	      mat[i][j] = 0.0;
      }
      else
      {			/* copy the col from its compressed location */
	    for(i = 0; i < size; i++)
