...
void mulAllocateBlockOfAllocationList( ssystem* sys );
...
/* int autom;			ON => choose depth automatically */
{
  ...
  sysALLOC(indsys->sys,1,ssystem, ON, AMSC, indsys,  AllocTypessystem );
  sys=indsys->sys;
  sys->indsys= (void*)indsys;
...
/* int flag;			ON => set depth automatically */
{
  ...
  if(flag == ON)
  {		/* set depth of partitions automatically */
    /* alloc spine of cube pntr array - leave enough room for depth = MAXDEP */
    mulALLOC(cubes, MAXDEP+1, cube****, ON, AMSC, sys, AllocTypePtrArray);

    /* allocate for levels 0, 1, and 2 (always used) */
    for(side = 1, i=0; i <= 2; side *= 2, i++)
    {
      mulALLOC(cubes[i], side, cube***, ON, AMSC, sys, AllocTypePtrArray);
      for(j=0; j < side; j++)
      {
	    mulALLOC(cubes[i][j], side, cube**, ON, AMSC, sys, AllocTypePtrArray);
	    for(k=0; k < side; k++)
        {
	       mulALLOC(cubes[i][j][k], side, cube*, ON, AMSC, sys, AllocTypePtrArray);
	    }
      }
    }
    /* side /= 2; */
	...
	mulALLOC(cubes[i], side, cube***, OFF, AMSC, sys, AllocTypePtrArray);
    if(cubes[i] == NULL)
    {
	...
	for(j=0; j < side; j++)
    {
	  mulALLOC(cubes[i][j], side, cube**, OFF, AMSC, sys, AllocTypePtrArray );
      if(cubes[i][j] == NULL)
      {
	    fprintf(stderr, "placeq: %d levels set up\n", i-1);
	    exit(1);
	  ...
	  for(k=0; k < side; k++)
      {
	    mulALLOC(cubes[i][j][k], side, cube*, OFF, AMSC, sys, AllocTypePtrArray);
	    if(cubes[i][j][k] == NULL)
        {
	      fprintf(stderr, "placeq: %d levels set up\n", i-1);
	      exit(1);
	...
	if(nextc == NULL)
    {
	  mulALLOC(nextc, 1, cube, OFF, AMSC, sys, AllocTypecube);
      if(nextc == NULL)
      {
	    fprintf(stderr, "placeq: %d levels set up\n", i-1);
	    exit(1);
	  ...
	  mulALLOC(nextc->upnumeles, 1, int, OFF, AMSC, sys, AllocTypeGeneric);
  ...
  {			/* old code - uses sys->depth for depth */
    /* Allocate the cubes, note calloc used because zeros everything. */
    depth = sys->depth;
    mulALLOC(cubes, sys->depth+1, cube****, ON, AMSC, sys, AllocTypePtrArray);
...
*/
    ...
    for(nextq = charges; nextq != NULL; nextq = nextq->next) {
      ...
      if(nextc == NULL)
      {
    	mulALLOC(nextc, 1, cube, ON, AMSC, sys,AllocTypecube );
        cubes[depth][xindex][yindex][zindex] = nextc;
	    nextc->upnumvects = 1;
	    mulALLOC(nextc->upnumeles, 1, int, ON, AMSC, sys, AllocTypeGeneric);
	    nextc->upnumeles[0] = 1;
      }
      else
      {
  ...
  mulALLOC(sys->q, totalq+1, double, ON, AMSC, sys, AllocTypeGeneric);
  mulALLOC(sys->p, totalq+1, double, ON, AMSC, sys, AllocTypeGeneric);

  /* set up mask vector: is_dummy[i] = TRUE => panel i is a dummy */
  mulALLOC(sys->is_dummy, totalq+1, int, ON, AMSC, sys, AllocTypeGeneric);


  /* set up mask vector: is_dielec[i] = TRUE => panel i is on DIELEC or BOTH */
  mulALLOC(sys->is_dielec, totalq+1, int, ON, AMSC, sys, AllocTypeGeneric);
  ...
  for(j=0; j < side; j++)
  {
    for(k=0; k < side; k++)
    {
      for(l=0; l < side; l++)
      {
        ...
        if(nextc != NULL)
        {  /* Only fill out nonempty cubes. */
	       /* Allocate for the charge ptrs, and get q vector pointer. */
	       mulALLOC(nextc->chgs, nextc->upnumeles[0], charge*, ON, AMSC, sys, AllocTypePtrArray);
           mulALLOC(nextc->upnumeles, 1, int, ON, AMSC, sys, AllocTypeGeneric);
	     ...
	     for(l=0; l < side; l++)
	     {
	       ...
	       if(nextc != NULL)
	       {
	         ...
	         if(i < sys->depth)
             {
	           ...
	           mulALLOC(nextc->kids, nextc->numkids, cube*, ON, AMSC, sys, AllocTypePtrArray);
	         ...
	         if(i > 0)
             {
	           ...
	           if(parent == NULL)
               {
		         mulALLOC(parent, 1, cube, ON, AMSC, sys, AllocTypecube);
...
void indexkid(ssystem *sys, cube *dad, int *pqindex, int *pcindex)
{
  ...
  if(dad != NULL)
  {
    ...
    {
      mulALLOC(dad->upvects, 1, double*, ON, AMSC, sys, AllocTypePtrArray);
      mulALLOC(dad->nbr_is_dummy, 1, int*, ON, AMSC, sys, AllocTypePtrArray);
  ...
  for(i=depth; i > 0; i--, side /= 2) {
	...
	for(l=0; l < side; l++) {
	  ...
	  if(nc != NULL) {
		...
		nc->mul_exact = TRUE;
		nc->upnumvects = 1;
		mulALLOC(nc->upvects, 1, double*, ON, AMSC, sys, AllocTypePtrArray);
		mulALLOC(nc->upnumeles, 1, int, ON, AMSC, sys, AllocTypeGeneric);

		nc->upnumeles[0] = num_eval_pnts; /* was numchgs 30Mar91 */
		nc->multisize = num_eval_pnts; /* was numchgs */
		mulALLOC(nc->chgs, num_eval_pnts, charge*, ON, AMSC, sys, AllocTypePtrArray);
		...
		for(m=0, first=TRUE; m < nc->numkids; m++)
        {
		  ...
		  if(nkid != NULL)
          {
		    if(first == TRUE)
            {
              ...
              {
			    mulALLOC(nc->nbr_is_dummy, 1, int*, ON, AMSC, sys, AllocTypePtrArray);
              }
  ...
  for(i = 1, side = 2; i <= depth; i++, side *= 2) {
	...
	for(l=0; l < side; l++) {
	  ...
	  if(nc != NULL) {
	    ...
	    if(nc->numnbrs > 0)
        {
           mulALLOC(nc->nbrs, numnbrs, cube*, ON, AMSC, sys, AllocTypePtrArray);
        }
...
void linkcubes(ssystem *sys)
{
  ...
  mulALLOC(sys->multilist, sys->depth+1, cube*, ON, AMSC, sys, AllocTypePtrArray);
  mulALLOC(sys->locallist, sys->depth+1, cube*, ON, AMSC, sys, AllocTypePtrArray);
      ...
      for(k=0; k < side; k++)
      {
	    for(l=0; l < side; l++)
        {
	      ...
	      if(nc != NULL)
          {
	        ...
	        if(i > 1)
            {		/* no multis over the root cube and its kids */
	          if(nc->mul_exact == FALSE)
              { /* exact -> mul_exact 1Apr91 */
		        mulALLOC(nc->multi, numterms, double, ON, AMSC, sys, AllocTypeGeneric);
		        *pmnc = nc;
		        pmnc = &(nc->mnext);
	         }
	        }
	       ...
	       if(i > 1)
           {		/* no locals with level 0 or 1 */
	         if(nc->loc_exact == FALSE)
             { /* exact -> loc_exact 1Apr91 */
		       *plnc = nc;
		       plnc = &(nc->lnext);
		       mulALLOC(nc->local, numterms, double, ON, AMSC, sys, AllocTypeGeneric);
	         }
	       }
...
int getInter(cube *child, ssystem *sys)
{
  ...
  if(vects > 0)
  {
    mulALLOC(child->interList, vects, cube*, ON, AMSC, sys, AllocTypePtrArray);
  }
  for(j = 0; j < vects; j++)
  {
...
{
  ASSERT(sys!=NULL)
  ASSERT(sys->indsys!=NULL)
  sysALLOC_func(PNTR, NUM, TYPESIZE, FLAG, MTYP,ATYP,(SYS*)sys->indsys, filename,linenumber);
}
