...
void indPrecond(ssystem *sys, SYS *indsys, double w)
{
  ...
  static int *fillist=NULL;      /* list of the filament numbers to which rows */
                            /* and cols of mat correspond */
  static int *findx=NULL;        /* For every filament, -1 if not in fillist, row */
  ...
  static double **nmat = NULL;
  static int *is_in_nc = NULL;
  static DUPS *is_dup = NULL;
  static int *is_partial = NULL;
  ...
  if (mat == NULL)
  {
    ...
    CALLOC(mat, maxsize, double*, ON, AMSC);
    CALLOC(fillist, maxsize, int, ON, IND);   /* filament numbers  IND stuff */
    for(i=0; i < maxsize; i++)
    {
      CALLOC(mat[i], maxsize, double, ON, AMSC);
    }
  }
  ...
  for(nc=sys->directlist; nc != NULL; nc = nc->dnext)
  {
    ...
    {     /* loop on neighbors of nc */
      ...
      if(NEAR(nnbr, nj, nk, nl))
      {
	    ...
	    for(noffset = 0, l = -1; l < nc->numnbrs; l++)
	    { /* lp on nc's nbrs */
	      if(l < 0)
	        nnnbr = nc;
	      else
	      ...
	      if(NEAR(nnnbr, nj, nk, nl)) {  /* Note, near to nc!! */
	        ...
	        { /* Find this nnnbr's position in nnbr's list */
  ...
  /* FastHenry stuff */
	...
	default:
      ...
      {
        fprintf(stderr,"What kind of precondtioner? %u\n", indsys->precond_subtype);
        exit(1);
      }
    }
  #endif
...
/*      if (ntrips >= 100 && !(trips % (ntrips/10))) {
    ...
    for(i = 0; i < meshsize; i++)
    {
	  ...
	  for(j = 0; j < meshsize; j++)
	  {
        ...
        {
          realmcol = meshnum[j];
          elem = (CX *)spGetElement(Matrix,realmrow+1,realmcol+1);
          elem->real += meshmat[i][j].real;
