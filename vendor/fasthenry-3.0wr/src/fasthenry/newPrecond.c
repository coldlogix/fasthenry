...
/* This function changes indsys->precond_type,indsys->precond_subtype */
    ...
    fill_diagL(sys, indsys, w);
      ...
      for(i = 0; i < num_mesh; i++)
      {
        for(j = i; j < num_mesh; j++)
        {
          ...
          if (used == 1)
          {
              elem1 = (CX *)spGetElement(Matrix,i+1,j+1);
              elem2 = (CX *)spGetElement(Matrix,j+1,i+1);

	          elem1->real = elem2->real = rtempsum;
    ...
    for(i = 0; i < num_fils; i++)
    {
      ...
      for(j = 0; j < num_fils; j++)
      {
	    ...
	    for(mtranj=Mtrans[filj]; mtranj != NULL; mtranj = mtranj->mnext)
        {
          ...
          {
            elem = (CX *)spGetElement(Matrix,mtrani->filindex+1,mtranj->filindex+1);
